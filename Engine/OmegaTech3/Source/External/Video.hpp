...
static inline int64_t atomic_cmpxchg(thread_atomic_int64_t *atomic,
                                     int64_t expected, int64_t desired) {
  return __sync_val_compare_and_swap(&atomic->i, expected, desired);
}

static inline bool atomic_cmpxchg_expect(thread_atomic_int64_t *atomic,
                                         int64_t expected, int64_t desired) {
...
static inline int64_t atomic_spin_till_expected(thread_atomic_int64_t *atomic,
                                                int64_t expected) {
  int64_t spins = 0;
...
static inline thread_ptr_t thread_create(int (*thread_proc)(void *),
                                         void *user_data, size_t stack_size) {
  pthread_t thread;
  if (0 !=
...
static int ray_video_thread_worker(void *user) {
  ...
  bool ok = atomic_cmpxchg_expect(&ctx->thread_state, THREAD_STATE_WORK,
                                  THREAD_STATE_IDLE);
  return ok ? 0 : 1;
}
...
ray_video_t ray_video_create_by_file_handle(FILE *file) {
  ...
  if (file) {
    ...
    video.ctx = (ray_video_thread_context_t *)calloc(
...
int ray_video_update(ray_video_t *video, double deltatime_s) {
  ...
  if (state == RAY_VIDEO_UPDATE_STATUS_NEW_FRAME) {
    ...
    if (video->texture.id == 0) {
      ...
      video->texture.id = rlLoadTexture(video->ctx->rgb_buffer, video->width,
                                        video->height, format, video->mipmaps);
