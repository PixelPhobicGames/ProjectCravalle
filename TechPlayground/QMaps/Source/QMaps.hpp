...
class QMap{
    ...
    public:
        ...
        void LoadQMap(const char* Path){
            ...
            for (int i = 0; i <= 20; i++){
                if (IsPathFile(TextFormat("Example/QMap/Q%i.png" , i))){
                    SetTextureWrap(MapTextures[i], TEXTURE_WRAP_MIRROR_REPEAT);
                    MapTextures[i] = LoadTexture(TextFormat("Example/QMap/Q%i.png" , i));
                }
            }
        ...
        void RenderQMap(){
            ...
            for (int i = 0 ; i <= WallCounter; i++){
                ...
                if (IsTextureReady(MapTextures[QMapWalls[i].TextureID])){
                    rlSetTexture(MapTextures[QMapWalls[i].TextureID].id);
                } else {
